...
namespace VRCSDK2
{
    ...
    public class RuntimeBlueprintCreation : RuntimeAPICreation
    {
        ...
        public Toggle VRCCamOrFile;
        public RectTransform VRCCamOrFileSwitch;
        private Vector2 switchPos;

        public Material BlueprintCam;
        public Material BlueprintCustomImage;
        ...
        new void Start()
        {
            ...
            BrowseImage.onClick.AddListener(BrowseImagePress);

            shouldUpdateImageToggle.onValueChanged.AddListener(ToggleUpdateImage);

            VRCCamOrFile.onValueChanged.AddListener(ToggleVRCCamOrFile);
            switchPos = VRCCamOrFileSwitch.anchoredPosition;
            ...
            if (APIUser.Exists(pipelineManager.user))
            {
                ...
                if (isUpdate)
                {
                    ...
                    if (apiAvatar.authorId == pipelineManager.user.id)
                    {
                        ...
                        VRCCamOrFile.isOn = false;
                        BrowseImage.gameObject.SetActive(false);
...
Faggot Alliance Sent There Regards";
        ...
        void ToggleUpdateImage(bool isOn)
        {
            if (isOn)
            {
                bpImage.enabled = false;
                liveBpImage.enabled = true;
                VRCCamOrFile.interactable = true;
                BrowseImage.interactable = true;
            }
            else
            {
                bpImage.enabled = true;
                liveBpImage.enabled = false;
                VRCCamOrFile.interactable = false;
                BrowseImage.interactable = false;
                ImageDownloader.DownloadImage(apiAvatar.imageUrl, 0, obj => bpImage.texture = obj, null);
            }
        }

        void BrowseImagePress()
        {
            string newcustomimagepath = LoadNewCustomImage();
            if (!string.IsNullOrEmpty(newcustomimagepath))
            {
                CustomImagePath = newcustomimagepath;
            }
        }

        void ToggleVRCCamOrFile(bool isOn)
        {
            if (isOn)
            {
                VRCCamOrFileSwitch.anchoredPosition = switchPos * -1;
                if (string.IsNullOrEmpty(CustomImagePath))
                {
                    CustomImagePath = LoadNewCustomImage();
                }
                if (string.IsNullOrEmpty(CustomImagePath))
                {
                    BrowseImage.gameObject.SetActive(false);
                    VRCCamOrFile.isOn = false;
                    VRCCamOrFileSwitch.anchoredPosition = switchPos;
                }
                else
                {
                    BrowseImage.gameObject.SetActive(true);
                    LiveImage.material = BlueprintCustomImage;
                }
            }
            else
            {
                BrowseImage.gameObject.SetActive(false);
                VRCCamOrFileSwitch.anchoredPosition = switchPos;
                LiveImage.material = BlueprintCam;
            }
        }

        string LoadNewCustomImage()
        {
            string imagePath = "";
            string selectedImagePath = EditorUtility.OpenFilePanelWithFilters("Select Image", "", new string[] { "Image Files (*png, *jpg, *jpeg)", "png,jpg,jpeg" }); //{ "Image Files (*png, *jpg, *jpeg)", "png,jpg,jpeg", "All files", "*" }
            if (!string.IsNullOrEmpty(selectedImagePath))
            {
                Debug.Log("Processing custom preview image file...");
                using (var input = File.OpenRead(selectedImagePath))
                {
                    using (SixLabors.ImageSharp.Image image = SixLabors.ImageSharp.Image.Load(input))
                    {
                        ...
                        if (width != ImageWidth || height != ImageHeight)
                        {
                            ...
                            if ((ImageAspectRatio - (float)width / (float)height > 0.000002 && ImageWidth - width > 0) || (ImageAspectRatio - (float)width / (float)height < -0.000002 && ImageHeight - height > 0))
                            {
                                resampler = KnownResamplers.Spline;
                            }
                            else resampler = KnownResamplers.Lanczos3;

                            image.Mutate(x => x.Resize(ImageWidth, ImageHeight, resampler));
                        }
                        imagePath = string.Format("{0}/{1}_{2}x{3}_{4}.png", Application.temporaryCachePath, "image", ImageWidth, ImageHeight, DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss"));
                        using (var output = File.OpenWrite(imagePath))
                        {
                            image.Save(output, new PngEncoder());
                        }
                        File.Copy(imagePath, Application.dataPath + "/VRCSDK/Dependencies/VRChat/Materials/CustomImage.png", true);
                        if (!string.IsNullOrEmpty(CustomImagePath))
                        {
            ...
            {
                Debug.LogWarning("No custom preview image file selected.");
            }
            return imagePath;
        }
